#!/usr/bin/env bash
# Pre-Release Validation Script for CausalGo - Causal Analysis Library
...
echo ""
echo "================================================"
echo "  CausalGo - Pre-Release Check"
echo "================================================"
echo ""
...
COVERAGE=$(go test -cover ./... 2>&1 | grep "coverage:" | head -1 | awk '{print $5}' | sed 's/%//')
if [ -n "$COVERAGE" ]; then
...
# 9. Critical dependencies check
log_info "Checking critical dependencies..."
DEPS_MISSING=0

# Check gonum
GONUM_VERSION=$(grep "gonum.org/v1/gonum" go.mod | awk '{print $2}')
if [ -n "$GONUM_VERSION" ]; then
    echo "  • gonum: $GONUM_VERSION"
else
    log_error "gonum dependency not found in go.mod"
    DEPS_MISSING=1
fi

# Check scigolib/matlab
MATLAB_VERSION=$(grep "github.com/scigolib/matlab" go.mod | awk '{print $2}')
if [ -n "$MATLAB_VERSION" ]; then
    echo "  • scigolib/matlab: $MATLAB_VERSION"
else
    log_error "scigolib/matlab dependency not found in go.mod"
    DEPS_MISSING=1
fi

# Check causalgo/lasso
LASSO_VERSION=$(grep "github.com/causalgo/lasso" go.mod | awk '{print $2}')
if [ -n "$LASSO_VERSION" ]; then
    echo "  • causalgo/lasso: $LASSO_VERSION"
else
    log_error "causalgo/lasso dependency not found in go.mod"
    DEPS_MISSING=1
fi
...
DOCS_MISSING=0
REQUIRED_DOCS="README.md CHANGELOG.md LICENSE CODE_OF_CONDUCT.md CONTRIBUTING.md SECURITY.md"
OPTIONAL_DOCS="ROADMAP.md"
